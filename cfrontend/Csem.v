...
Module Csem (P: Policy) (A: Allocator P).
  ...
  | cast_args_nil:
    cast_arguments l pct fpt m Enil Tnil (Success (pct, []))
  | cast_args_cons: forall pct' pct'' v vt vt' ty el targ1 targs v1 vl,
      ArgT l pct fpt vt (exprlist_len el) targ1 = Success (pct', vt') ->
      sem_cast v ty targ1 m = Some v1 ->
      cast_arguments l pct' fpt m el targs (Success (pct'',vl)) ->
  ...
  | cast_args_fail_now: forall v v1 vt ty el targ1 targs msg failure,
      ArgT l pct fpt vt (exprlist_len el) targ1 = Fail msg failure ->
      sem_cast v ty targ1 m = Some v1 ->
      cast_arguments l pct fpt m (Econs (Eval (v,vt) ty) el) (Tcons targ1 targs)
                     (Fail msg failure)
  | cast_args_fail_later: forall pct' v vt vt' ty el targ1 targs v1 msg failure,
      ArgT l pct fpt vt (exprlist_len el) targ1 = Success (pct', vt') ->
      sem_cast v ty targ1 m = Some v1 ->
      cast_arguments l pct' fpt m el targs (Fail msg failure) ->
      cast_arguments l pct fpt m (Econs (Eval (v,vt) ty) el) (Tcons targ1 targs)
                     (Fail msg failure)
  .
  ...
  Section EXPR.
    ...
    Inductive lfailred: expr -> control_tag -> trace -> string -> FailureClass -> Prop :=
    | failred_field_struct: forall ofs pt id co a f ty pct delta bf msg failure,
        ce!id = Some co ->
        field_offset ce f (co_members co) = OK (delta, bf) ->
        FieldT l ce pct pt ty id = Fail msg failure ->
        lfailred (Efield (Eval (Vlong ofs, pt) (Tstruct id a)) f ty) pct E0
                 msg failure
    | failred_field_union: forall ofs pt id co a f ty pct delta bf msg failure,
        ce!id = Some co ->
        union_field_offset ce f (co_members co) = OK (delta, bf) ->
        FieldT l ce pct pt ty id = Fail msg failure ->
    ...
    | red_call_internal: forall pct pct' b vft tyf m tyargs tyres cconv el ty fd vargs,
        ...
        type_of_fundef fd = Tfunction tyargs tyres cconv ->
        classify_fun tyf = fun_case_f tyargs tyres cconv ->
        cast_arguments l pct vft m el tyargs (Success (pct',vargs)) ->
