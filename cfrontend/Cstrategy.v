...
Module Cstrategy (T: Tags) (P: Policy T) (A: Allocator T P).
  Module Ctyping := Ctyping T P A.
  Import Ctyping.
  Import Csem.
  Import Csyntax.
  Import Cop.
...
Lemma safe_imm_safe:
  forall f l PCT C a k e te m K,
  safe (ExprState f l PCT (C a) k e te m) ->
  context K RV C ->
...
Lemma lred_kind:
  forall l e a te m PCT a' te' m', lred ge ce l e a te m PCT a' te' m' -> expr_kind a = LV.
Proof.
  induction 1; auto.
Qed.
...
Lemma rred_kind:
  forall l PCT a m e t PCT' a' e' m', rred ge ce l PCT a e m t PCT' a' e' m' -> expr_kind a = RV.
Proof.
  induction 1; auto.
Qed.
...
Lemma safe_expr_kind:
  forall from C f l PCT a k e te m,
  context from RV C ->
  safe (ExprState f l PCT (C a) k e te m) ->
  expr_kind a = from.
Proof.
