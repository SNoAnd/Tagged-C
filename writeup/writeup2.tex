...
oprotection that isn't relevant elsewhere.
...
No compartmentalization is possible without keeping track of which compartment is in control at
any given time. In the presence of function pointers, this may not be trivial! \cref{fig:functions}
shows, step by step, how a typical function call progresses and how the tag on the function pointer
is ultimately fed to the \(\calltname\) tag rule. At return, the \(\rettname\) rule can access the
old PC tag from before the call. In a compartmentalization policy, the PC tag identifies the current
function, so \(\calltname\) will set the PC tag to \(\mathit{comp}(id)\) and \(\rettname\) will
restore it to its value before the call.

\begin{figure}
  Call(f) -> Call(p@FN_f) -> CallState(p) -> f.s

  Return v -> Retstate -> Wherever
...
\begin{table}
  \begin{center}
    %\begin{tabular}{ c| c |c }
    \title{Further Policies and the Uses}
    \begin{tabular}{  p{3cm} | p{5cm}| p{5cm}  } 
      ...
      Temporal Memory Safety & Prevent double free & Assign unique color to tag at each free. Check for an existing color before freeing. \\
