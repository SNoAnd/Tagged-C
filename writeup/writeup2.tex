...
\apt{A rather odd formulation!} The definition of what constitutes
...
Let's assume that we have a compartmentalization scheme\apt{what's that??}  that is being added to the system
after development. A set of compartment identifiers are ranged over by \(C\),
and function and global identifiers are mapped to compartments by \(\mathit{comp}(id)\). \apt{Last sentence out of place here.}
...
A variable expression where the variable is the function {\tt f} \apt{???} evaluates to \(\mathtt{f}@\FN[\mathtt{f}]\).
...
and a memory safety policy. \apt{Probably need to bring back some of the commented text here.}
...
\apt{Suggest removing this subsection and labeling the discussion by topic --- tags vs. values; memory tagging; etc. --- giving pros and cons for each
  much as the text currentlly does.}
