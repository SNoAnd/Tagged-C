...
\item Tagged C policies enforcing (1) compartmentalization, \apt{can we say anything about the novelty of the public/private model? PRISC doesn't count as a prior publication!}
...
the expression {\tt \&f} evaluates to \(\alpha\tagat\FN[f]\). \apt{sean please check my change}
...
optional \apt{why?} label that is part of Tagged C's abstract syntax. This way, the policy can react when execution
...
constructor {\sc \color{blue} pc}, which carries the current function identifier\apt{why??}  and
...
takes this label (optionally, since not all policies will care about join points.\apt{this is still
  mysterious. (a) I thought the optional part was because there might \emph{be} no join point.
  (b) otherwise, even if the policy doesn't care, why wouldn't we instrument the program?})
...
\begin{figure}[t]
  \begin{align*}
    & \mem \in \mathit{MEM} ::= \{ \mathit{contents}; \mathit{perms}; \mathit{globals}; \mathit{live}; \mathit{allocator} \} & \textnormal{where} \\
    & \mathit{frag} ::= (v\tagat\vt)_n & n \in \mathbb{N} \\
    & \mem.\mathit{contents} \in \Int \rightarrow \mathit{frag}\tagat \lt \\
...
\begin{figure}[t]
  ...
  \begin{cases}
    v\tagat\vt\tagat\left[\lt[_0]\dots\lt[_n]\right] & \textnormal{if } \mem.\mathit{contents} ~ (p+n) = (v\tagat\vt)_n\tagat\lt[_n], \\
    & \textnormal{and } \mem.\mathit{perm} ~ (p+n) \not = \mathit{Reserved}, \textnormal{ where } 0 \leq n < |\type| \\
    \vundef\tagat\vt\tagat\left[\lt[_0]\dots\lt[_n]\right] & \textnormal{else if } \mem.\mathit{contents} ~ (p+n) = \underline{~~}\tagat\lt[_n] \\
    & \textnormal{and } \mem.\mathit{perm} ~ (p+n) \not = \mathit{Reserved}, \textnormal{ where } 0 \leq n < |\type| \\
  ...
  \begin{cases}
    \left\{ \mem.\mathit{contents}\llbracket p + n \mapsto (v\tagat\vt)_n\tagat(\mathit{nth} ~ n ~ \overline{\lt}) \mid \rrbracket 0 \leq n < |\type| \right\} \\ \hspace{4em} \textnormal{if } \mathit{perm} ~ (p+n) \not = \mathit{Reserved} \textnormal{ where } 0 \leq n < |\type| \\
