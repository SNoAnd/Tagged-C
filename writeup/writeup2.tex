...
These layers may be shored up with dynamic security enforcement. Reference monitors 
are a popular mechanism for dynamic enforcement. Unfortunately,
their semantics are often underspecified, leaving gaps,
...
for a built-in {\em tag-based reference monitor} based on PIPE (Processor
Interlocks for Policy Enforcement). Tagged C expresses security 
policies at a level familiar to C developers: that
...
also based on that of CompCert~\cite[Ch. 4]{Leroy22:CompCertManual}, for use in prototyping and executing policies. Tag rules are written directly
...
runtime monitors that are more directly comparable to Tagged C. Here, the advice
that is interleaved
...
goal of also extending CompCert's semantics preservervation guarantees to cover tagged semantics.
...
it leaves the possibility that new policies will arise for which our current set of
control points is inadequate.  There is no conceptual reason why
the control points cannot be extended on demand, but extending the interpreter
and, even more
and even more the compilerthe overhead of doing this
will obviously increase once we have a compiler---particularly because of the need
to consider how tag rule evaluation interacts with compiler optimziation~\cite{Chhak21:Tagine}.
It is also important to keep the ultimate 
...
\subsection*{Heap Memory Tagging}
...
variant names (\cref{sec:compartments}) is something of a hack. A more principled solution
  ...
  static points within functions, but we have so far intentionally avoided this in
  favor of a ``black box'' approach to policy construction.
  ...
  versions of {\tt malloc} that take more informative parameters. While this is
  not satisfactory for protecting legacy code, we do not see a good alternative at present.
