...
\begin{figure}[t]
  ...
  \begin{align*}
    \mem \in \mathit{MEM} ::= \{ & \contents : \Int \rightarrow \mathit{frag}\tagat \tau, \\
...
\paragraph{Globals and the Global Environment}

\begin{figure}[t]
  \[\genv ::= \GN \rightharpoonup (\Int \times \Int \times \tau \times \type)\]
  \[\fenv ::= \FN \rightharpoonup (\alpha \times \mathit{fdef})\]

  \caption{Environments}
  \label{fig:genv}
\end{figure}

Globals are stored in memory in a single contiguous region. A global
...
the end of its range), its pointer tag, and its type.
