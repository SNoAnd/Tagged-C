...
\begin{abstract}
\apt{Still reads awkwardly, and doesn't expose problem/solution clearly.}
...
is a programmable hardware mechanism for supporting reference monitors 
at the granularity of individual instructions. 
Each value in memory and each register, well understood objects in hardware,
is extended with a metadata tag. At the start of executing an instruction,
PIPE checks the opcode and the tags on its operands to see if the operation 
should be permitted according to tag rules, and if so, what tags should be 
assigned to the result. A collection of tag rules forms a PIPE policy.


PIPE, the specific motivator for our work, is particularly flexible: its design supports
arbitrary software-defined flow rules over large (word-sized) tags with arbitrary structure,
which enables fine-grained policies and the composition of multiple policies. This flexibility 
...
\apt{Are we supplying the Coq code as an artifact?}
...
in this case \(\accesstname\) and \(\assigntname\), are omitted.\apt{recheck; if still valid, explain more.}}
...
by the kind of operation as well as the tags on its inputs.\apt{Both $\binoptname$ and $\argtname$ also take $\PCT$ here, but not in the example figure.
  Need to make these agree; if we keep $\PCT$ here, need to give a forward-ref to its definition.}
...
assignment under a high PC remains high.\apt{Why isn't this shown in the figure??}
...
The other known limitation is in how we tag memory at allocation time. Each call to {\tt malloc} tags an
...
based on its type, but in C, {\tt malloc} does not get access to type information---it allocates
memory of a given size and returns a {\tt void *}, and it is up to the caller to cast the
pointer to the desired type. That cast may be arbitrarily distant from the call to {\tt malloc}!
This is a well-known impediment to improving C memory safety, and previous work has often
adopted approaches similar to ours; for example, Michael et al.
\cite{Michael23:MSWASM} replacing {\tt malloc} outright 
...
\subsection{Fixed set of Control Points}

There is always the possibility that we have missed some control point
