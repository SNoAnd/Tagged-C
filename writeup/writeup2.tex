...
local variable {\tt x}. We now explain how Tagged C can enforce a security policy that would prevent this leak.
...
\Cref{fig:ex2} steps through an execution of {\tt g}.
...
Tagged C is full ordinary C. Its semantics (given in full in the appendix)
...
\apt{It wouldn't hurt to talk briefly through these rules, especially when the connection between rule name and control point is not obvious. And make a point of saying: ``and this is all there are or ever will be (we hope)''}
...
The other known limitation is in how we tag memory at allocation time.
