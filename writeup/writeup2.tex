...
Security can be improved by using runtime reference monitors, which might
be implemented using hardware support, such as the PIPE (Processor
Interlocks for Policy Enforcement) processor extensions. Unfortunately,
existing PIPE-based systems require policies to be expressed at the ISA level,
which makes them opaque and limits their applicability to higher-level language features.
To address this problem, we introduce Tagged C, a novel C variant with formalized and verified semantics 
for a built-in {\em tag-based reference monitor} based on PIPE.
Tagged C expresses security policies at a level familiar to C developers: that
...
It supports different approaches to security and varying levels of strictness. We demonstrate
...
\item Tagged C policies enforcing (1) compartmentalization.
  (2) realistic memory models (PVI and PNVI),
...
\item A full formal semantic definition for Tagged C, formalized in Coq, describing how the control points
  interact with programs, and interpreter, implemented in Coq and extracted to OCaml.
\end{itemize}
...
The policy described above is implemented by instantiating the tag rules as shown in \cref{fig:example1rules}. 
In tag rules,
...
active compartment. Secure information flow (SIF) policies expose the many places
where the user may need to reference identifiers from their program in the policy itself.
Together, these example policies illustrate Tagged C's breadth of application.
...
While the idea seems obvious, the definition of a valid pointer can vary. The C standard
...
CompCert C's definition of a valid pointer allows the pointer to be cast into an integer and back, provided that its value
...
For simplicity this example has omitted expression conditionals, but the approach is similar.
...
Initially, the PC tag is \(\pctaint{\FN[f]}{\varepsilon}\), which corresponds
...
then interleaves all of the advice with its relevant advice points.
...
the system---it cannot interfere in the execution except to failstop. This means that
...
means that PIPE acceleration will not be feasible.
...
Formally, a memory's contents is a map from machine integer addresses to
byte-sized fragments. For a value \(v\) of type \(\type\) and tag \(\vt\),
\((v\tagat\vt)_n\) is the \(n\)th fragment of \(v\tagat\vt\). We write
\(\mem[p]_{\type} = v\tagat\vt\tagat\overline{lt}\) to indicate that \(\mem[p+i] = (v\tagat\vt)_i\)
for \(0 \leq i < |\type|\).
