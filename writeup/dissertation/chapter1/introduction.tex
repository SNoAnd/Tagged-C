...
The class of tag-based policies includes a number of important security concepts, with implementations
...
specification, and verification of policies. This dissertation addresses those challenges.
In the next section we will walk through two example policies to understand how they
are written, why doing so is challenging, and what might be done about it.

\section{Tag Policies By Example}

This section shows two example policies: a simplified ``spatial'' stack safety
policy, and an information flow control policy. The goal is not to introduce these policies
on a deep techical level, but to give an intuition for how they might be attached to an assembly
program and why this is a complex process.

In brief, a policy consists of a collection of rules, each associated with a family of opcodes.
Almost all policies will need to distinguish individual special instructions via tags on their values
in memory, since many opcodes can play different roles that need to be
...
memory safety \cite{Azevedo+16}, a given policy may not precisely match it. In the interest of simplicity,
this example only offered spatial safety, while stack safety is more commonly thought of as having
a temporal dimension. If it were meant to be more than an illustrative example, spatial stack safety
should be formally defined so that the policy can be shown to enforce it. Then it will be up to the user
whether that level of security is sufficient. In a more realistic setting the policies would not
sacrifice protection for simplicity, but it still might due to performance constraints.
...
block and then restored to its old value once it leaves. In the expanded code in Figure \ref{ch1:ex:if2},
...
proven correct at that level. In the case of the Figure \ref{ch1:ex:if} example, it matters that the code
...
tags to the level of a source language. In this dissertation that language is C. Figure \ref{ch1:fig:language}
