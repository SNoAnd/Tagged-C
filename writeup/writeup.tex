...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       {
         \node (l1) {\tt void main(int pwd) \{}; \\
         \node (l2) {\tt ~ ~ int x = pwd;}; \\
         \node (l3) {\tt ~ ~ printf("\%d", x);}; \\
         \node {\tt \}}; \\
       };

       \node[node distance=20em, right=of l1.west, draw=black, anchor=north]           (te1)     {\(\mathtt{pwd} \mapsto 42\)};
       \node[node distance=1em, right=of te1, draw=black]                        (p1)      {\(\vundef\)};
       \node[node distance=0.5em, above=of p1.west]                              (pname1)  {\tt x};

       \node[node distance=20em, right=of l2.west, draw=black, anchor=north]           (te2)     {\(\mathtt{pwd} \mapsto 42\)};
       \node[node distance=1em, right=of te2, draw=black]                        (p2)      {\(42\)};


       \node[node distance=1em, below=of l3.east, draw=black, anchor=north]                        (call)    {\(\mathit{Call} ~ \mathtt{printf} ~ (\texttt{"\%d"}, 42)\)};
...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       ...
       \node[matrix, node distance=18em, right=of l1.west, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&, anchor=north] (te1)
            {\node {\(\mathtt{pwd} \mapsto 42@\)}; \& \node (t1) {\(\PT\)}; \\};

       \node[matrix, node distance=1em, right=of te1, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&] (p1)
            {\node {\(\vundef@\)}; \& \node (t2) {\(\N\)}; \\};
            
       \node[node distance=0.5em, above=of p1.west]                              (pname1)  {\tt x};

       \node[node distance=4em, below=of te1, draw=black, anchor=north]     (te2)     {\(\mathtt{pwd} \mapsto 42@\PT\)};
       \node[matrix, node distance=1em, right=of te2, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&] (p2)
            {\node {\(42@\)}; \& \node (t3) {\(\PT\)}; \\};

       \node[node distance=0.5em, above=of p2.west]                              (pname2)  {\tt x};


        \node[matrix, node distance=8em, below=of l3.east, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&, anchor=north] (call)
            {\node {\(\mathit{Call} ~ \mathtt{printf} ~ (\texttt{"\%d"}, 42 @\)}; \& \node (t4) {\(\color{blue} \square\))}; \\};

       \node[node distance=2em, below=of te1.east, draw=blue] (trule1) {\(\storetname\)};
       \node[node distance=2em, below=of te2.east, draw=blue] (trule2) {\(\argtname\)};

       \draw[-{Triangle},thick,draw=blue]
       (t1.south) -- (trule1);
...
{\em control points}. Collectively, the tag rules form a {\em policy}.

In example \ref{fig:ex2}, we update {\tt pwd} in place, and the \(\binoptname\) tag rule
checks the tags on both it and the constant being added to it. Since one side of the
expression is tagged \(\PT\), that must be propagated, even though the other side is \(\N\).
Once again, \(\argtname\) will failstop.
...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       ...
       \node[right=of te1] (t2) {5@\(\N\)};
            
       \node[matrix, node distance=4em, below=of te1, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&, anchor=north] (te2)
            {\node {\(\mathtt{pwd} \mapsto 47@\)}; \& \node (t3) {\(\PT\)}; \\};

        \node[matrix, node distance=8em, below=of l3.east, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&, anchor=north] (call)
            {\node {\(\mathit{Call} ~ \mathtt{printf} ~ (\texttt{"\%d"}, 42 @\)}; \& \node (t4) {\(\color{blue} \square\))}; \\};

       \node[node distance=1em, above=of t2, draw=blue] (trule0) {\(\consttname\)};
       \node[node distance=2em, below=of te1.east, draw=blue] (trule1) {\(\binoptname\)};
       \node[node distance=2em, below=of te2.east, draw=blue] (trule2) {\(\argtname\)};
...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       ...
       \node[matrix, node distance=1em, right=of te1, column sep=0em, inner sep=0.2em,
         draw=black, ampersand replacement=\&] (p1)
            {\node {\(\vundef@\)}; \& \node (t2) {\(\N\)}; \& \node[draw=black] (t3) {@ \(\M\)}; \\ };
            
       \node[node distance=1em, above=of p1.west]                              (pname1)  {\tt mm};
       ...
       \node[node distance=0.5em, above=of p2.west]                              (pname2)  {\tt mm};
...
Finally, we may be concerned with implicit leaks, such as the one in
example \ref{fig:ex4}. In order to prevent this,
...
written \(\PCT\). When we perform the check to enter the for loop, {\tt i<pwd},
the resulting value is tagged \(\PT\) (due to the \(\binoptname\) rule above.)
We consult the \(\splittname\) tag rule, and it tells us that \(\PCT\) should
be \(\PT\) as well (abbreviated \(\PCT'\) for space.)
We extend \(\storetname\) to check the PC Tag as well
as the value being written, and failstop if either is \(\PT\) and the memory
location is \(\M\).
...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       {
         \node {\tt volatile int mm;}; \\
         \node (l0) {\tt void main(int pwd) \{}; \\
         \node (l1) {\tt ~ ~ for (int i=0; i<pwd; i++)}; \\
         \node (l2) {\tt ~ ~ ~ ~  mm = i;}; \\
         \node {\tt \}}; \\
       };

       \node[matrix, node distance=16em, right=of l2.west, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&] (te1)
            {\node {\(\mathtt{pwd} \mapsto 42@\)}; \& \node (t1) {\(\PT\)}; \\
              \node {\(\mathtt{i} \mapsto 0@\)}; \& \node (t1) {\(\N\)}; \\
            };

       \node[state, node distance=1em, left=of te1, draw=black] (pct1)
            {\(\PCT'\)};
       \node[state, node distance=1em, above=of pct1, draw=black] (pct0)
            {\(\N\)};
       ...
       \node[matrix, node distance=1em, right=of te2, column sep=0em, inner sep=0.1em,
         draw=black, ampersand replacement=\&] (p2)
            {\node {\(42@\)}; \& \node (t4) {\(\square\)}; \& \node[draw=black] (t5) {@ \(\square\)}; \\};

       \node[node distance=0.5em, above=of p2.west]                              (pname2)  {\tt mm};

       \node[node distance=2em, below=of te1.east, draw=blue] (trule1) {\(\storetname\)};
       \node[node distance=2em, right=of pct0.east, draw=blue] (trule0) {\(\splittname\)};

       \node[state, node distance=1em, left=of te2, draw=black] (pct2)
            {\(\square\)};
...
\begin{tikzpicture}
  \argtruleblock{\(\mathbf{assert ~ \PCT \sqcup \vt \sqsubseteq (f, x)}\)}{}
\end{tikzpicture}
\begin{tikzpicture}
  \rettruleblock{\(\mathbf{assert} ~ \PCT \sqcup \vt \subseteq \)}{}{}
\end{tikzpicture}
...
with the user's Tagged C policy. \amn{automatically run?}
