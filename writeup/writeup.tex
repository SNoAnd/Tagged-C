...
% Use a letter other that P for pwd
% Discuss the four examples first, explain what kind of checks we'd like to do
% Maybe better: one routine that leaks four different ways
...
\begin{tikzpicture}[every text node part/.style={align=left}]
  \node[matrix, ampersand replacement=\&, anchor=west]
       ...
       \node[node distance=20em, right=of l1.west, draw=black, anchor=north]     (te1)     {\(\mathtt{pwd} \mapsto 42@\PT\)};
       \node[node distance=1em, right=of te1, draw=black]                        (p1)      {\(\vundef @\N\)};
       \node[node distance=0.5em, above=of p1.west]                              (pname1)  {\tt x};

       \node[node distance=20em, right=of l2.west, draw=black, anchor=north]     (te2)     {\(\mathtt{pwd} \mapsto 42@\PT\)};
       \node[node distance=1em, right=of te2, draw=black]                        (p2)      {\(42@\PT\)};

       \node[node distance=1em, below=of te2.west, draw=black, anchor=north west]                        (call)    {\(\mathit{Call} ~ \mathtt{printf} ~ (\mathtt{"\%d"}, 42 @ \vt)\)};
...
could deduce {\tt pwd} by subtraction. Consider example \ref{fig:ex2}, focusing first on
...
% explain rule here; how do we handle M?
