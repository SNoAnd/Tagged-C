...
two tags, and an expression. The first tag is that of the outermost context (ultimately,
the PC Tag of the state), while the second is that of the innermost, which changes when
we enter an parenthetical with an optional tag.

\[\begin{aligned}
\mathit{context} & ~ \PCT ~ \gentag ~ k ~ \ctx{\expr} ::= \\
| & \mathit{context} ~ \PCT ~ \gentag ~ k ~ \lambda \expr.\expr & \\ % ctx_top
| & \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \lambda \expr.\deref{\ctx{\expr}} &
\textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \\ % ctx_deref
| & \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \lambda \expr.\field{\ctx{\expr}}{id} &
\textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \\ % ctx_field
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\valof{\ctx{\expr}} &
\textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \ctx{\expr} \\ % ctx_rvalof
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\addrof{\ctx{\expr}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \ctx{\expr} \\ % ctx_addrof
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\unop{\odot}{\ctx{\expr}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \\ % ctx_unop
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\binop{\oplus}{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_binop_left
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\binop{\oplus}{\expr_1}{\ctx{\expr_2}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_2} \\ % ctx_binop_right
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\cast{\ctx{\expr}}{\type} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \\ % ctx_cast
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\seqand{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_seqand
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\seqor{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_seqor
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\condition{\ctx{\expr_1}}{\expr_2}{\expr_3} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_condition
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\assign{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \ctx{\expr_1} \\ % ctx_assign_left
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\assign{\expr_1}{\ctx{\expr_2}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_2} \\ % ctx_assign_right
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\assignop{\oplus}{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \ctx{\expr_1} \\ % ctx_assignop_left
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\assignop{\oplus}{\expr_1}{\ctx{\expr_2}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_2} \\ % ctx_assignop_right
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\postinc{\oplus}{\ctx{\expr}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \lh ~ \ctx{\expr} \\ % ctx_postinc
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\call{\ctx{\expr_1}}{\overline{\expr_2}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_call_left
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\call{\expr_1}{\ctx{\overline{\expr_2}}} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \textnormal{ for } \expr \in \overline{\expr_2} \\ % ctx_call_right
% skipped builtins
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\comma{\ctx{\expr_1}}{\expr_2} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr_1} \\ % ctx_comma
| & \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \lambda \expr.\paren{\ctx{\expr}}{\type}{} & \textnormal{where } \mathit{context} ~ \PCT ~ \gentag ~ \rh ~ \ctx{\expr} \\ % ctx_paren
| & \mathit{context} ~ \PCT ~ \gentag' ~ \rh ~ \lambda \expr.\paren{\ctx{\expr}}{\type}{\gentag} & \textnormal{where } \mathit{context} ~ \gentag ~ \gentag' ~ \rh ~ \ctx{\expr} \\ % ctx_paren
\end{aligned}\]

This means that in a tagged parenthetical, the tag entirely supersedes the current PC Tag. This means that
any information from the PC Tag must carry over onto the tag on the parenthetical. For this reason,
parentheticals only have tags as an intermediate form, to make sure that they capture the current
PC Tag when they're constructed.
