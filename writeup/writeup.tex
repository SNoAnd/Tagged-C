...
Operating systems (Linux, Windows, OSX, BSD), databases (Oracle, sqlite3), the internet \&
 ...
 C is not a relic; more than a third of professional programmers report active developing
  in C today \cite{stackoverflow22:dev-survey}. The security of C code is synonymous 
  with the safety of public and private systems we depend on every day. 

  Although the security of C code is critical to public safety, 
  it is not always possible to fully secure it before run-time. 
  Ideally, bugs would be identified and then fixed promptly. 
  That is not always possible for a variety of reasons: bugs may escape detection, 
  escape identification, or be unchangeable. Static analysis is a well researched 
  technique for identifying bugs, however it suffers from a fatal flaw: C is not 
  ...
  Unfortunately the distinction is typically lost on static analysis tools. 
  ...
  rarely perfectly successful \cite{https://dl.acm.org/doi/10.1007/978-3-642-36563-8_14 }
   even if such an expert is available at all. Even when there is both consensus 
   and detection of a bug, changes to the code may not be possible. 
   It may be resident in proprietary 3rd party libraries and drivers, or in code
   where regulations prohibit changes \cite{coverity}.

The last line of defense against unknown or immutable vulnerabilities is dynamic enforcement in 
the runtime. Dynamic enforcement options are well studied, however most suffer from similiar 
flaws; they cannot easily discern developer intent nor are usable by C developers,
i.e. the source of developer intent. We propose Tagged C, a dynamic tag-based policy enforcement 
system operating at the same level the developers do: the C source code itself. Tagged C allows
developers to express their security intent for UB from permissive to conservative. 


[TODO: old intro. remove after new intro is added]
...
It is not possible to enforce this statically in general, though it may be over-approximated.
...
We need a way to dynamically track the value of {\tt pwd} as it moves through the variable
...
Finally, we may be concerned with less leaks of {\tt pwd}, such as this one:
\begin{verbatim}
void main(int pwd) {
  for (int i; i<pwd; i++) {
    printf(i);
...
To this end, we carry an additional tag associated with the global state.
[TODO: diagram here, too, except I'm not sure how to effectively represent the added complexity.]

\paragraph{General Purpose Security Enforcement}

[TODO: Emphasize breadth]
...
of the array pointed to by {\tt x}. In Tagged C under a typical allocation strategy,
  ...
  \node[matrix, ampersand replacement=\&, column sep=-1em, node distance=0.5em, below=of stmt]  (mem)
       {
         ...
         \node[anchor=center] {\memory{\(92\)}{\(\vundef\)}{\(\N\)}{\(\tagz\)}}; \&
         \node[anchor=center] {\memory{\(96\)}{\(\vundef\)}{\(\N\)}{\(\tagone\)}}; \\
       };
...
\begin{tikzpicture}[every text node part/.style={align=center}]
  ...
  \node[matrix, ampersand replacement=\&, column sep=-1em, node distance=0.5em, below=of stmt]  (mem)
       {
         \node[anchor=north] {\(~ ~\cdots ~ ~\)}; \&
         \node[anchor=center] (x) {\(\underbrace{\memory{\(92\)}{\(0\)}{\(\N\)}{\(\tagz\)}}\)}; \&
         \node[anchor=center] {\(~ ~ \cdots ~ ~\)}; \\
       };

  \storetruleblock{\(\forall \lt \in \lts . \pt = \lt\)}{\(\PCT\)}{\(\vt_2\)}{\(\lts\)}
...
\begin{verbatim}
  void configure_enterprise(char* intended_name) {
    int ret = 0;
    char curr_name = [HOST_NAME_MAX + 1] ;
