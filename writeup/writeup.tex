...
monitor~{\cite{Anderson72:PlanningStudy}. In particular, a large class
  ...
  one-size-fits-all restrictions.\apt{Would be good to expand on why
    this flexibility is useful.} But PIPE tags and rules are defined
...
%% with PIPE as a likely hardware target. PIPE is distinctive compared to similar systems such as ARM MTE~\cite{arm-mte}
%% and STAR~\cite{Gollapudi+23} in that its tags are very large---typically the size of words
...
%% specifying detailed secure information flow policies, and enforcing arbitrary mandatory access control rules. % OK to still say this when we aren't showing it?
...
Consider a seemingly\apt{??} straightforward security requirement: ``do not leak high-security values to low-security channels.''
...
that uses the lower-order bits of a pointer to store a flag% (\cref{fig:mark})
should execute successfully. This idiom appears often in code where a pointer is associated with a flag,
such as Cheney's garbage collection algorithm \cite{} and is generally considered harmless.
But violations such as buffer overflows in which a load, store, or free would
occur outside of the proper range are undefined in PVI and must failstop. In \cref{fig:overrun},
...
\Cref{fig:overrun} shows the initial state of memory on entry to {\tt overflow} and the state after
...
have \(\pt_1 = \lt_1 = \tagz\) and \(\pt_2 = \lt_2 = \tagone\). When we try to write to {\tt x+2},
...
calling \(f'\). In the initial call step, \(\calltname\) uses the tag on the function pointer
...
allocated and tagged with the results of \(\localtname\). On return, locals are deallocate, their locations
...
{\em mandatory access control} (MAC) and {\em capabilities}. MAC explicitly
...
\begin{figure}
  \begin{minipage}[t]{0.6\textwidth}
    \malloctruleblock
        {let \((C,\clr) := \PCT\) in \\
          \caseoftwo{\(f'\)}
                    {\(\mathit{malloc}\)}{\(\PCT' := \PCT; \pt := C;\) \\ & & \(\vt := \N; \lt := C\)}
                    {\(\mathit{malloc\_share}\)}{\(\PCT':= (C,\clr+1);\) \\ & & \(\pt := \clr; \vt := \N';\) \\ & & \(\lt := \clr\)}
...
\begin{figure}
  ...
  \begin{minipage}{0.45\textwidth}
    \loadtruleblock
        {let \(\pctaint{f}{d}{Ls} := \PCT\) in \\
          \caseoftwo{\(f\)}
                    {\(\mathit{sql\_query}\)}{\(\mathbf{assert} ~ \vt \sqcap \pt = \high\) \\ & & \(\vt' := \vt\)}
