...
On the flip side, we have this code, where a malformed user input is accidentally appended
to an sql query without sanitization:
...
These are both instances of problematic ``information-flow'' that can be prevented
using a {\em secure information flow} (SIF) policy in Tagged C, similar to our very
first example. SIF is described in the venerable Denning and Denning
\cite{Denning77:SecureInformationFlow}, and part of a larger family of policies
known as {\em information flow control} (IFC.) At its simplest, if we classify inputs and outputs to
