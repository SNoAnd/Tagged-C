...
%   loss of life: https://ieeexplore.ieee.org/document/274940 bug in C, medical device killed people
...
The policy described so far isolates memory errors, and prevents memory sharing outright,
but otherwise puts no restrictions on the interactions between compartments. We can go a
step further and build in a {\em Mandatory Access Control} (MAC) table, restricting each
compartment's ability to call into other compartments and access globals.

\paragraph{Memory Shared by Capability}

The above policy is very restrictive! Compartments can never share memory with one
another---they can pass pointers, but the pointers are functionally just integers
until they're passed back. This is obviously unsatisfying. What can we do?

Assuming that we know {\em which} objects should be shared, we can implement a hybrid
system in which shared objects follow rules resembling standard fine-grained memory
safety, while objects that are not intended to be shared default to coarse-grained
protection.

For example, suppose we have a function {\tt foo\_factory} that allocates an object,
initializes it, and then returns its pointer. We want its caller to be able to use that pointer,
but by default, we don't want an outside function accessing the other internal data of
the compartment.
...
In this case, the idiomatic Tagged C way to distinguish {\tt bar} from other allocations is
to mark its allocation point with a label. The name tag associated with the label will be
fed to the \(\labelt\) rule.
