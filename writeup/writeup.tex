...
\apt{Maybe start with ``Although a wide variety of static analyses have been applied to detect and
  mitigate C insecurities,''}
The last line of defense against unknown or unfixable vulnerabilities is dynamic enforcement at
runtime. To this end\apt{to what end? The actual need being addressed needs to be made explicit here!}, we introduce Tagged C: a general-purpose dynamic tag-based enforcement language
that allows developers to define flexible security policies in terms of a familiar source language.
Applications of Tagged C include giving precise definition to undefined behaviors (UBs) involving memory, specifying detailed information flow policies, and \apt{more here ???}.
The user can apply whatever security policy meets their needs,
tuned if necessary so that non-standard but benign code can still run\apt{this is a little mysterious in this context}, while actually dangerous activity
is halted. Importantly, it is rarely necessary to recompile code to modify a security policy.
\apt{omit: ``after the fact''. And by the way, when \emph{is} it necessary?}

The novelty of Tagged C is in building a tag-based reference monitor directly into the C semantics.
\apt{I hope not.  The novelty is in having a \emph{general-purpose} scheme for specifying monitor behavior
  based on tags. The semantics is just an artifact of giving a rigorous definition to this scheme.}
...
{\em tag rules}. Our underlying semantics are built on the CompCert C semantics, which are formalized 
...
\sna{I feel some need to justify this\apt{what, exactly?}, but I'm not sure what justification is appropriate.}

\apt{Need to say \emph{something} about PIPE or other enforcement mechanisms in the intro.}
...
\item Tagged C policies implementing (1) compartmentalization,
  (2) realistic, permissive memory models from the literature (PVI and PNVI\apt{maybe}),
...
\apt{Sorry, but I really find these diagrams hard to follow and needlessly complicated.
  I'm sure we have had better ways of describing these things in the past.}

Consider this state diagram of example \ref{fig:ex1}. In the temporary environment,
we map {\tt pwd} to 42, and then assign it into {\tt x} in memory.

\begin{tikzpicture}[every text node part/.style={align=center}]
  \node[state]                                        (stmt)   {\tt x = pwd};
  \node[node distance=0.5em, above=of stmt, draw=black] (te)     {\(\mathtt{pwd} \mapsto 42\)};
  \node[node distance=1em, below=of stmt, draw=black] (p)      {\(\vundef\)};
  \node[node distance=0.5em, above=of p.west]                (pname)  {\tt x};

  \node[state, right=of stmt]                         (stmt2)  {\(\sskip\)};
  \node[node distance=0.5em, above=of stmt2, draw=black] (te2)    {\(\mathtt{pwd} \mapsto 42\)};
  \node[node distance=1em, below=of stmt2, draw=black] (p2)     {\(42\)};
  \node[node distance=0.5em, above=of p2.west]                (pname2) {\tt x};

  \node[state, right=of stmt2]                         (stmt3)  {\tt printf \\ \tt ("\%d", x)};
  \node[node distance=0.5em, above=of stmt3, draw=black] (te3)    {\(\mathtt{pwd} \mapsto 42\)};
  \node[node distance=1em, below=of stmt3, draw=black] (p3)     {\(42\)};
  \node[node distance=0.5em, above=of p3.west]                (pname3) {\tt x};
...
\begin{tikzpicture}[every text node part/.style={align=center}]
  \node[state]                                        (stmt)   {\tt x = pwd};
  \node[node distance=0.5em, above=of stmt, draw=black] (te)     {\(\mathtt{pwd} \mapsto 42 @ \PT\)};
  \node[node distance=1em, below=of stmt, draw=black] (p)      {\(\vundef @ \N\)};
  \node[node distance=0.5em, above=of p.west]                (pname)  {\tt x};

  \node[state, right=of stmt]                         (stmt2)  {\(\sskip\)};
  \node[node distance=0.5em, above=of stmt2, draw=black] (te2)    {\(\mathtt{pwd} \mapsto 42 @ \PT\)};
  \node[node distance=1em, below=of stmt2, draw=black] (p2)     {\(42 @ \PT\)};
  \node[node distance=0.5em, above=of p2.west]                (pname2) {\tt x};

  \node[state, right=of stmt2]                         (stmt3)  {\tt printf \\ \tt ("\%d", \\ \tt x)};
  \node[node distance=0.5em, above=of stmt3, draw=black] (te3)    {\(\mathtt{pwd} \mapsto 42 @ \PT\)};
  \node[node distance=1em, below=of stmt3, draw=black] (p3)     {\(42 @ \PT\)};
  \node[node distance=0.5em, above=of p3.west]                (pname3) {\tt x};

  \node[node distance=6em, right=of stmt3] (center) {};
  
  \node[state,node distance=2em, above=of center]      (call)  {\(\mathit{Call}\dots\) \\ \(\mathtt{42}@\N\)};
  \node[node distance=2em, below=of center]      (fail)  {\(\fail\)};
...
%    \node[node distance=5em, right=of x, state]        (printf) {\tt printf \\ (``\%d'', 42);};
  ...
  \path[->]
  (stmt) edge node [pos=0.5,below] {\(\gentag \rightarrow \gentag\)} (stmt2)
  (stmt2) edge (stmt3)
  (stmt3) edge node [pos=0.5,below,sloped] {\(\N \rightarrow \N\)} (call)
  (stmt3) edge node [pos=0.5,below,sloped] {\(\PT \rightarrow \fail\)} (fail);
\end{tikzpicture}
\vspace{\belowdisplayskip}
...
could deduce \(\PT\) by subtraction. Consider example \ref{fig:ex2}, focusing first on
the computation of {\tt pwd + 5} inside of the context \(C\), then moving on to make
the call once the expression is fully evaluated.

\begin{tikzpicture}[every text node part/.style={align=center}]
  \node[state]                                        (stmt)   {\(C[\mathtt{pwd}+\) \\ \(5@\N]\)};
  \node[node distance=0.5em, above=of stmt, draw=black] (te)     {\(\mathtt{pwd} \mapsto 42 @ \PT\)};

  \node[state, right=of stmt]                         (stmt2)  {\(C[42@\PT+\) \\ \(5@\N]\)};

  \node[state, node distance=6em, right=of stmt2]                         (stmt3) {\(\ctx{47@\PT}\)};

%  \node[node distance=6em, right=of stmt3] (center) {};
  
%  \node[state,node distance=2em, above=of center]      (call)  {\(\mathit{Call}\dots\) \\ \(\mathtt{47}@\N\)};
%  \node[node distance=2em, below=of center]      (fail)  {\(\fail\)};
...
%    \node[node distance=5em, right=of x, state]        (printf) {\tt printf \\ (``\%d'', 42);};
  ...
  \path[->]
  (stmt) edge (stmt2)
  (stmt2) edge node [pos=0.5,below]
       {\(\N,\N \rightarrow \N\) \\
         \(\PT,\gentag \rightarrow \PT\) \\
         \(\gentag,\PT \rightarrow \PT\)} (stmt3);

  \node[state, node distance=1em, below=of stmt3]            (stmt4) {\(\mathtt{printf}\) \\ \tt ("\%d", \\ \tt \(47@\PT\))};
  \node[node distance=6em, right=of stmt4] (center) {};

  \node[state,node distance=2em, above=of center]      (call)  {\(\mathit{Call}\dots\) \\ \(\mathtt{47}@\N\)};
  \node[node distance=2em, below=of center]      (fail)  {\(\fail\)};
  
  \path[->]
  (stmt4) edge node [pos=0.5,below,sloped] {\(\N \rightarrow \N\)} (call)
  (stmt4) edge node [pos=0.5,below,sloped] {\(\PT \rightarrow \fail\)} (fail);
    
\end{tikzpicture}

Function calls aren't the only means of leaking data, however. We might additionally want
to prevent {\tt pwd} from leaking by being written mmapped memory, like in example \ref{fig:ex3}.
Supposing the {\tt mm} is
such a location, we would separately tag the location itself (not just the value in the
location) to designate it as such. We'll call that tag \(\M\).

\vspace{\abovedisplayskip}
\begin{tikzpicture}[every text node part/.style={align=center}]
  \node[state]                                        (stmt)   {\tt x = pwd};
  \node[node distance=0.5em, above=of stmt, draw=black] (te)     {\(\mathtt{pwd} \mapsto 42 @ \PT\)};
  \node[node distance=0em, below=of stmt] (p)      {\(\memory{}{\(\vundef\)}{\(\N\)}{\(\N\)}{}\)};
  \node[node distance=0.5em, above=of p.west]                (pname)  {\tt x};
  \node[node distance=-0.5em, below=of p] (mm)      {\(\memory{}{\(\vundef\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm.west]                (mmname)  {\tt mm};

  \node[state, right=of stmt]                         (stmt2)  {\(\sskip\)};
  \node[node distance=0em, below=of stmt2] (p2)      {\(\memory{}{42}{\(\PT\)}{\(\N\)}{}\)};
  \node[node distance=0.5em, above=of p2.west]                (pname2)  {\tt x};
  \node[node distance=-0.5em, below=of p2] (mm2)      {\(\memory{}{\(\vundef\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm2.west]                (mmname2)  {\tt mm};

  \node[state, right=of stmt2]                         (stmt3)  {\tt mm = x};
  \node[node distance=0em, below=of stmt3] (p3)      {\(\memory{}{42}{\(\PT\)}{\(\N\)}{}\)};
  \node[node distance=0.5em, above=of p3.west]                (pname3)  {\tt x};
  \node[node distance=-0.5em, below=of p3] (mm3)      {\(\memory{}{\(\vundef\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm3.west]                (mmname3)  {\tt mm};
 
  \node[node distance=12em, right=of stmt3] (center) {};
  
  \node[state,node distance=2em, above=of center]      (write)  {\(\sskip\)};
  \node[node distance=-0.5em, below=of write] (mm3)      {\(\memory{}{\(42\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm3.west]                (mmname3)  {\tt mm};
  \node[node distance=2em, below=of center]      (fail)  {\(\fail\)};
...
%    \node[node distance=5em, right=of x, state]        (printf) {\tt printf \\ (``\%d'', 42);};
  ...
  (stmt) edge node [pos=0.5,below] {\(\gentag \rightarrow \gentag\)} (stmt2)
  (stmt2) edge (stmt3)
  (stmt3) edge node [pos=0.5,below,sloped] {\(\gentag,\N \rightarrow \gentag\) \\ \(\N,\M \rightarrow \N\)} (write)
  (stmt3) edge node [pos=0.5,below,sloped] {\(\PT,\M \rightarrow \fail\)} (fail);
\end{tikzpicture}
\vspace{\belowdisplayskip}

Finally, we may be concerned with implicit leaks of {\tt pwd}, such as the one in
example \ref{fig:ex4}. It will give away the value of {\tt pwd}! In order to prevent this,
we must keep track of the privilege of the current context, by carrying
an additional tag associated with the global state. This is called the PC Tag,
written \(\PCT\). Inside of a loop or conditional that depends on {\tt pwd}, the
state carries information about {\tt pwd}, and this is reflected by \(\PCT = \PT\).
In the example below we focus on the state inside of the loop, where \(\PCT = \PT\),
and so we should not write to {\tt mm}.

\begin{minipage}{0.6\textwidth}
\begin{tikzpicture}[every text node part/.style={align=center}]
  \node[state, right]                         (stmt3)  {\tt mm = x \\ \(\PCT = \PT\)};
  \node[node distance=0em, below=of stmt3] (p3)      {\(\memory{}{42}{\(\PT\)}{\(\N\)}{}\)};
  \node[node distance=0.5em, above=of p3.west]                (pname3)  {\tt x};
  \node[node distance=-0.5em, below=of p3] (mm3)      {\(\memory{}{\(\vundef\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm3.west]                (mmname3)  {\tt mm};
 
  \node[node distance=10em, right=of stmt3] (center) {};
  
  \node[state,node distance=2em, above=of center]      (write)  {\(\sskip\)};
  \node[node distance=-0.5em, below=of write] (mm3)      {\(\memory{}{\(i\)}{\(\N\)}{\(\M\)}{}\)};
  \node[node distance=0.5em, above=of mm3.west]                (mmname3)  {\tt mm};
  \node[node distance=2em, below=of center]      (fail)  {\(\fail\)};
...
%    \node[node distance=5em, right=of x, state]        (printf) {\tt printf \\ (``\%d'', 42);};
  ...
  \path[->]
  (stmt3) edge node [pos=0.5,below,sloped] {\dots} (write)
  (stmt3) edge node [pos=0.5,below,sloped] {\(\PT,\gentag,\M \rightarrow \fail\) \\
    \(\gentag,\PT,\M \rightarrow \fail\)} (fail);
\end{tikzpicture}
\end{minipage}
...
\apt{The purpose of this section is unclear.  
  Are you trying to introduce the syntax or the semantics of Tagged C?  Formally or informally?
  The points of difference from CompCert are hard to appreciate without a lot more (at least informally)
  about the standard CompCert semantics.  Without at least some semantic rules, you lack a way
  to describe how control points are hooked in.
  Since you are giving almost no details now, the stuff on notation seems way too pedantic.}
...
of each tag rule identified in \cref{fig:controlpoints}. We divide these into the most common (light gray),
which cover operations such as loads, stores, and arithmetic; more specialized ones (dark gray) that
deal with state initialization and special functions; and the cast rules (red) which are necessary
for certain niche policies.\apt{Actually, I think colors is overkill: just the re-ordering is enough.}
...
\apt{Next two paras are making a roadblock out of a small point. Shorten and/or defer discussion of options.}
...
\begin{itemize}
\item Function identifiers, \(FN(f)\) \apt{For decent kerning, use $\mathit{FN}$, etc. rather than $FN$.}
...
Next, the program stores a 0 to address 1000\apt{92(?)}. The constant 0
takes on the default tag, \(I\)\apt{??}. The policy needs to check that
...
byte being written, in case the pointer is misaligned.\apt{unclear} Then the tag on the value being stored
is propagated with it into memory, unchanged.
\apt{explain what StoreT means}
...
\begin{tikzpicture}[every text node part/.style={align=center}]
  \localtruleblock{}{\(\PCT+1\)}
                  {\(\mathit{dyn ~ \PCT}\)}
                  {\(\N\)}
                  {\(\left[ \mathit{dyn} ~ \PCT \right]\)}
\end{tikzpicture}
\begin{tikzpicture}[every text node part/.style={align=center}]
  \malloctruleblock{}{\(\PCT+1\)}
                   {\(\mathit{dyn ~ \PCT}\)}
                   {\(\N\)}
                   {\(\left[ \mathit{dyn} ~ \PCT \right]\)}
\end{tikzpicture}
