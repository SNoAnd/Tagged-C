...
\address{Portland State University, Department of Computer Science}
\email{ander28@pdx.edu, naak@pdx.edu, apt@pdx.edu}
...
Operating systems (Linux, Windows, OSX, BSD), databases (Oracle, sqlite3), the internet \&
 ...
 C is not a relic; more than a third of professional programmers report active developing
  in C today \cite{stackoverflow22:dev-survey}. The security of C code is synonymous\apt{too strong} 
  with the safety of public and private systems we depend on every day. 

  \apt{You need to talk here about the kinds of properties we plan to address.}


  \apt{This para gets in the way of the flow. We do not need to diss static enforcement to this audience.
    Also, the para makes it sound like we are only interested in C language errors. 
    But bugs in the programmer's logic are at least as important for the properties we are trying to enforce!}
  Although the security of C code is critical to public safety\apt{repeats previous sentence},
  it is not always possible to fully secure it before run-time.
  Ideally, bugs would be identified \apt{when?} and then fixed promptly. 
  That is not always possible for a variety of reasons: bugs may escape detection, 
  escape identification\apt{what's the difference}, or be unchangeable\apt{``unfixable''?} . Static analysis is a well researched \apt{and widely deployed!}
  technique for identifying bugs, but it suffers from a fatal flaw: C is not 
  well defined. There is not universal agreement among experts on what and when 
  something is a bug in C code, especially around Undefined Behaviors (UB) 
  \cite{defactoC}. Confusion around expected behavior is no small problem. 
  ...
  Unfortunately the distinction is typically lost on static analysis tools. \apt{I'm not sure
    this is a supportable statement.}
  ...
  and detection of a bug\apt{??}, changing the code may not be possible because  
  it is in proprietary 3rd party libraries and drivers, or because
  regulations prohibit changes \cite{Bessey10:Coverity}.\apt{last clause is mysterious}

\apt{This should be the second para. Move the stuff above to later.}
The last line of defense against unknown or immutable vulnerabilities is dynamic enforcement at
run time. Dynamic enforcement options are well studied\apt{citations?}, but most suffer from similar 
flaws: they cannot easily discern developer intent nor are usable by C developers,
i.e. the source of developer intent.\apt{awkward, and again needs evidence.}

In this paper, we propose Tagged C, a \apt{``generic'' or ``general-purpose''} dynamic tag-based policy enforcement 
system operating at \apt{unclear} the same level the developers do: the C source code itself. Tagged C allows
developers to express their security intent for UB from permissive to conservative, and even
beyond UB \apt{well beyond} to rule out defined-but-erroneous behaviors that may be difficult to predict.
...
is failstopped.\apt{not sure we want to coin this word.}

\apt{Need a high-level description of the elements of Tagged C here: control points, policy rules, etc.}
...
\paragraph{General Purpose Security Enforcement}
\apt{These points need to be integrated much earlier int he main text.}

Tagged C does not assume any particular notion of security. It can express a range of policies
including memory safety, compartmentalization, and secure information flow. These represent many
of the typical notions of security seen in the wild.
...
\paragraph{What is Metadata Tagging?}
\apt{If this really needs to be part of the intro, then it needs to come above the Contributions.}
...
It is not possible to enforce this statically in general, though it\apt{??} may be over-approximated.
\apt{Very unclear, and probably not needed here.}
...
We need a way to dynamically track the value\apt{``confidential status''?} of {\tt pwd} as it moves through the variable
...
Finally, we may be concerned with less \apt{??} leaks of {\tt pwd}, such as this one:
\begin{verbatim}
void main(int pwd) {
  for (int i; i<pwd; i++) {
    printf(i);
...
\apt{You still have not addressed my earlier comments about making this section more accessible, by:
  clarifying the difference between source and artificial intermediate forms in Figure 1, reordering al the
  figures to put the important/common cases first, etc.}
...
of the array pointed to by {\tt x}. In Tagged C under a typical allocation strategy\apt{what does that mean?},
{\tt x} and {\tt y} are given adjacent concrete addresses, and the program
writes to the address of {\tt x+1}, which is also the address of {\tt y}.
...
\begin{minipage}{0.49\textwidth}
  \[\begin{aligned}
  \truledef{\storet}
  \assert{\forall \lt \in \lts . \PCT \sqcup \pt \sqcup \vt_2 \sqsubseteq \lt}
  \settag{\PCT'}{\PCT}
