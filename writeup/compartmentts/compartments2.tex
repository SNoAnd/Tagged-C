...
\begin{figure}
  ...
  \caption{Reads and Writes \sna{Is the \(ms ~ id \mapsto\) syntax acceptable?}}
  \label{subfig:rwstep}
\end{figure}
...
\caption{Policy Definition \sna{Note that I added an ``f'' to the signature of RetT.
    This is the currently active function. We could track it via the PCTT, but it's easier
    if we just always have access to it.}
\label{fig:policy}
