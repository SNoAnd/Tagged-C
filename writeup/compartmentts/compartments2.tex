...
to each compartment, pointers to each shared object, and all other values. Location
tags mark addresses as being allocated to a particular compartment or shared object, or
else unallocated. The policy state is a counter that tracks the next allocation color.

\begin{figure}
\[\begin{aligned}
  & \tau_V ::= \mathtt{L}(C) | \mathtt{S}(a) | \emptyset &  \in \mathit{nat} \\
...
For simplicity, global variables are always local to a compartment. (Modifying the policy
to allow multiple compartments to share global variables would be straightforward.) Dynamic
memory 
...
instantiated with the policy above.

\paragraph{Bisimulation Proof}

We define the bisimulation relation \(\mathfrak{R}\) between states of the abstract machine and
states of Tagged C. We'll define the specific relation
...
\paragraph{Consquences of Bisimulation}
