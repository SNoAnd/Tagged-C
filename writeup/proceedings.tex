...
%security concerns.
\Cref{sec:related} discusses related work, and \cref{sec:futurework} describes limitations
and future work. The full formal Tagged C semantics appears in our appendix.
...
\end{tikzpicture}
\caption{Not an Implicit Flow\apt{missing usual background shading; spurious comma in first box(?)}}
\label{fig:sifex3}
\end{figure}
...
takes this label (optionally, since there might \apt{``not'' (??)} be a join point).
...
\section{Interpretation and Compilation}

In this work we focus on the source semantics of Tagged C, and give a proof of concept
in the form of a reference interpreter based on that of CompCert \cite[Ch. 4]{Leroy22:CompCertManual}.
It supports a restricted set of standard library functions.

The interpreter simulates micro-policies in software. Policies are written directly in
Galina, and the entire package of the interpreter and its policies are extracted
to Ocaml.

\subsection{Compilation to Tagged Assembly}

We have briefly introduced our previous work on Tagine, a verified compiler for a toy version
of Tagged C \cite{Chhak21:Tagine}. We envision a Tagged C compiler taking the same approach.
First, we need a summary of how PIPE policies work at the assembly level. At each step of execution,
the PIPE system consults a lookup table whose keys are a tuple of the following information:
\begin{itemize}
\item The ``group'' that the instruction belongs to, e.g. ``arithmetic operations,'' ``loads,''
  or ``stores,'' with each group sharing a signature
\item Tags on all of the inputs in the instruction group's signature, including any memory that
  it accesses
\item The PC tag
\item The tag on the instruction being executed
\end{itemize}

Entries in the table are the tags to place on the outputs of the operation, matching the
group's signature. If there is no entry for a particular set of tags, the system failstops.
In practice the lookup table may be implemented as a cache that does not hold all of the
combinations at once, but recomputes them on a miss; or it may be a fixed table supplemented
by dedicated circuits to perform simple operations, like equality comparison.

A compiler from Tagged C must apply a policy that protects the basic control-flow and
private data of the language (the ``baseline'' policy), and can also embed a high-level
policy. We will write low-level tags in orange\apt{doesn't show well in B\&W}, tags and rules inherited from the high-level
policy, still in blue. This embedding strategy centers around the low-level instruction tags.
Consider the simple function in \cref{fig:compilation}, and the result of compiling it to a simplified
RISC-V-like assembly with a non-optimizing compiler.

%When compiling a policy from Tagged C to run alongside
%a low-level policy with tag type \({\color{orange} \mathbb{T}_0}\), the combined policy
%has the tag type \(\mathbb{\color{orange} T} ::= {\color{orange} \mathbb{T}_0} \times
%{\color{blue} \tau}\), where \(\color{blue} \tau\) is the tag type of the source-level
%policy.
...
\end{verbatim}
  ...
  \begin{subfigure}[t]{0.79\textwidth}
    \begin{tabular}{|l l|}
      \hline
      {\tt 1  addi sp(-16),zero,sp}    & {\color{orange} @ Code,HeadSeq1}  \\
      {\tt 2  sd ra,sp(0)}             & {\color{orange} @ Code,HeadSeq2}  \\
      {\tt 3  addi zero(8),sp,t1}      & {\color{orange} @ Code,SrcVar({\tt \color{blue} x})} \\
      {\tt 4  addi zero(4),t1,t2}      & {\color{orange} @ Code,SrcRule(\(\binoptname(+)\))} \\
      {\tt 5  addi zero(42),zero,t0}   & {\color{orange} @ Code,SrcRule(\(\consttname\))} \\
      {\tt 6  sw t0,t1}                & {\color{orange} @ Code,SrcRule(\(\storetname \circ^{\vt\leftarrow\vt'}_{\PCT\leftarrow\PCT'} \assigntname\))} \\
      {\tt 7  addi zero(8),sp,t1}      & {\color{orange} @ Code,SrcVar({\tt \color{blue} x})} \\
      {\tt 8  addi zero(-4),t1,t2}     & {\color{orange} @ Code,SrcRule(\(\binoptname(-)\))} \\
      {\tt 9  addi zero(42),zero,t0}   & {\color{orange} @ Code,SrcRule(\(\consttname\))} \\
      {\tt 10 sw t0,t1}                & {\color{orange} @ Code,SrcRule(\(\storetname \circ^{\vt\leftarrow\vt'}_{\PCT\leftarrow\PCT'} \assigntname\))} \\
      {\tt 11 ld sp(0),ra}             & {\color{orange} @ Code,RetSeq1} \\
      {\tt 12 addi sp(16),zero,sp}     & {\color{orange} @ Code,RetSeq2} \\
      {\tt 13 jalr ra}                 & {\color{orange} @ Code,RetSeq3} \\
      \hline
    \end{tabular}
  \end{subfigure}

  \caption{A Simple Compilation Scheme}
  \label{fig:compilation}
\end{figure}

All of the code is tagged {\color{orange} Code}, and the baseline policy ensures that
only addresses with that tag will be executed. The header and return sequences are tagged
as such, and the baseline policy ensures that they are executed in order, and appropriately
tag the return address to protect it from being overwritten. The remaining code is tagged with
the {\color{orange} SrcRule} (or {\color{orange} SrcVar}) with the identity of the tag rule
that should execute along with that instruction. [TODO: initialize frame.]
In the case of loads and stores, the rules in question are the composition of two high-level rules,
with their outputs mapped to inputs as indicated.

In the example in \cref{fig:compilation}, the store at line 10 attempts to overwrite
the return pointer, and is prevented by the baseline policy regardless of the source
policy. But under a PNVI policy, the program failstops at line 6.

\paragraph{Tag sizes}

In modeling Tagged C, we assume unbounded tags. In practice, PIPE tags are assumed to be
word-sized. [Say something here about how big the baseline policy is.] That leaves
most of the tag size for the source-level policy. [Cite that one guy's work on the GC for tags.]
...
An AOP language distributes {\em join points}\footnote{Not to be confused with the control-flow graph join points
discussed in \cref{sec:SIF}.}
throughout its semantics, 
...
for formal proofs of its properties.  In particular, we would like to show that
