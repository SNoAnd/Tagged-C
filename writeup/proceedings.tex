...
%are special, because {\tt malloc} and {\tt free} are library functions.
...
by the \(\extcalltname\) tag rule.\apt{Right? please double-check}
...
\begin{figure}[t]
  ...
  \caption{Selected rules for Compartmentalization with Shared Capabilities, combining Compartmentalization rules (subscript \(C\), \cref{fig:compartments}) with Memory Safety rules (subscript \(\mathit{MS}\), \cref{fig:memgen})\apt{I made a change to line 6 on right; double-check!}}
  \label{fig:sharing}
\end{figure}
