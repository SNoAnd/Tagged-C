    ...
    \node[matrix, ampersand replacement=\&, node distance=6em, right=of code.north,draw] (table1)
         {
           ...
           \node[anchor=north, fill=blue!10] {\(\vt[_1] \leftarrow \argtname(\high,\FN[f],\AN[psk])\) \\ \(\vt[_2] \leftarrow \consttname\)};\\
         };
...
Suppose {\tt f} is called with an actual argument \(i \tagat \high\).
The tag on {\tt psk} is checked by \(\argtname\), which is also parameterized by the
names of both the function and the argument. The initial tag \(\vt[_2]\) on local
  ...
  \node[matrix, ampersand replacement=\&, node distance=6em, right=of l1.north,draw,anchor=west] (table1)  
       {
         ...
         \node[anchor=north, fill=blue!10] {\(\pt[_1], \vt[_1],\lt[_1] \leftarrow \globaltname(\GN[mm])\) \\
           \(\PCT[_1] \leftarrow \calltname(\low, \FN[g])\)  \\
           \(\vt[_2] \leftarrow \argtname(\high,\FN[g],\AN[psk])\)}; \\
       };
 ...
 \node[matrix, ampersand replacement=\&, node distance=6em, below=of l3,draw,anchor=west] (table3)         
      {
        ...
        \node[anchor=north, fill=blue!10] {\(\vt[_5] \leftarrow \consttname\) \\
...
at the point of call, which we assume to be \(\low\), is fed to \(\calltname\) to determine
a new PC tag
...
and the interpreter are then parameterized by a policy. For example, the code of the
PVI-memory-safety policy is shown in \cref{fig:fullcode}; it comprises only 60 lines of Gallina,
plus some boilerplate.

The interpreter simulates policies in software. The policies must be written directly in
...
\begin{figure}
  ...
  \begin{tabular}{l l}
    ...
    Variable ce : composite\_env. & Definition ConstT pct := PolSuccess N. \\

    & Definition InitT pct := PolSuccess N. \\
    
    Inductive tag := & Definition CallT pct pt := PolSuccess pct. \\
    | Glob (id:ident) & Definition ArgT pct vt f x := PolSuccess (pct,vt). \\
    | Dyn (c:nat) & Definition RetT pct\_clr pct\_cle vt := PolSuccess (pct\_cle,vt). \\
    | N. &     Definition FieldT pct vt ty id := PolSuccess vt. \\
    & Definition AccessT pct vt := PolSuccess vt. \\

    Definition def\_tag := N. & Definition AssignT pct vt1 vt2 := PolSuccess (pct,vt2). \\
    Definition InitPCT := Dyn 0. ~ &     Definition UnopT op pct vt := PolSuccess (pct, vt). \\ \\

  \end{tabular}
  
  \begin{tabular}{l}
    Definition BinopT (op: binary\_operation) (pct vt1 vt2: tag) := \\
    ~ ~ match vt1, vt2 with \\
    ~ ~ | Dyn n, X =>  PolSuccess (pct, vt1) \\
    ~ ~ | Glob id, X => PolSuccess (pct, vt1) \\
    ~ ~ | \_, \_ => PolSuccess (pct, vt2) \\
    ~ ~ end. \\ \\

    Definition LoadT (pct pt vt: tag) (lts : list tag) := \\
    ~ ~ match pt with \\
    ~ ~ | N => PolFail "PVI::LoadT N" ([pct;pt;vt]++lts) \\
    ~ ~ | \_ => if forallb (tag\_eq\_dec pt) lts then PolSuccess vt  \\
    ~ ~ ~ ~ else (PolFail "PVI::LoadT Ptr" ([pct;pt;vt]++lts)) \\
    ~ ~ end. \\ \\

    Definition StoreT (pct pt vt: tag) (lts: list tag) := \\
    ~ ~ match pt with \\
    ~ ~ | N => (PolFail "PVI::StoreT N" ([pct;pt;vt]++lts)) \\
    ~ ~ | \_ => if forallb (tag\_eq\_dec pt) lts then PolSuccess (pct,vt,lts) \\
    ~ ~ ~ ~ else (PolFail "PVI::StoreT Ptr" ([pct;pt;vt]++lts)) \\
    ~ ~ end. \\ \\

    Definition LocalT (pct: tag) (ty: type) := \\
    ~ ~ match pct with \\
    ~ ~ | Dyn c => PolSuccess (Dyn (S c), Dyn c, repeat (Dyn c) (sizeof ce ty)) \\
    ~ ~ | \_ => PolFail "PVI::LocalT Failure" [pct] \\
    ~ ~ end. \\ \\
  
    Definition DeallocT (pct: tag) (ty: type) := \\
    ~ ~ PolSuccess (pct, N, repeat N (Z.to\_nat (sizeof ce ty))). \\ \\

    Definition MallocT (pct pt vt: tag) := \\
    ~ ~ match pct with \\
    ~ ~ | Dyn c => PolSuccess (Dyn (S c), Dyn c, N, Dyn c) \\
    ~ ~ | \_ => PolFail "PVI::MallocT Failure" [pct;pt;vt] \\
    ~ ~ end. \\ \\

    Definition FreeT (pct pt vt: tag) (lts: list tag) := \\
    ~ ~ match pt with \\
    ~ ~ | Dyn c => if forallb (tag\_eq\_dec pt) lts then PolSuccess (pct, N, N). \\
    ~ ~ ~ ~ else PolFail ``PVI::FreeT Failure'' [pct;pt;vt] \\
    ~ ~ | \_, \_ => PolFail ``PVI::FreeT Failure'' [pct;pt;vt] \\
    ~ ~ end. \\ \\


    Definition PICastT (pct pt: tag)  (lts: list tag) (ty: type) := PolSuccess pt. \\
    Definition IPCastT (pct vt: tag)  (lts: list tag) (ty: type) := PolSuccess vt. \\
    Definition PPCastT (pct vt: tag) (lts: list tag) (ty: type) := PolSuccess vt. \\
    Definition IICastT (pct vt: tag) (ty: type) := PolSuccess vt. \\
  \end{tabular} }
...
for formal proofs of its properties. We prove the correctness of the interpreter with respect
to the Tagged C semantics, and we aim to prove properties of the policies.
