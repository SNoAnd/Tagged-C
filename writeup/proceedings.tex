...
A wrinkle comes in when we consider that variables can be private (non-address-taken)
or public (allocated in public memory). 
...
constraints can be placed on public memory accesses. For this reason, memory loads and stores
...
Function calls update the PC tag and can update the tags on any arguments,
while returns are given access to both the PC tag at the time of return and
the one that was active before the call.
    ...
    \malloctruleblock
      {let \({\color{blue} (C,\clr)} := \PCT\) in \\
        \caseoftwo{\(\pt\)}
                  ...
                  {\(\FN[malloc\_share]\)}
                  {\\\multicolumn{3}{l}{~~\({\color{blue} \clr'},\pt['],\lt['] \leftarrow {\color{blue} \mathbf{MallocT}_{\mathit{MS}}(\PCT,\pt,\vt)}\)} \\
