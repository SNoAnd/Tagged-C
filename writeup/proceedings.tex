...
To reprove the correctness of the interpreter with respect to the semeantics
require updating the proof automation to handle concrete memory and tags.
...
then extracted to Ocaml.
For example, \cref{fig:fullcode} shows part of the policy module's signature, its instantiation
in the PVI policy, and how those rules are accessed in the Coq semantics. The full
policy fits into 70 lines of Gallina.
...
private state (separate from the underlying application being monitored) over multiple
...
\begin{figure}[t]
  ...
  \caption{Instantiating Policy Module with PVI}
  \label{fig:fullcode}
\end{figure}
